...
\begin{definition}[Loss Function]\label{def:loss}
    ...
    \begin{equation*}\label{eq:loss}
        \altmathcal{L}: \altmathcal{H}_{\pmb{\Lambda}} \times \mathbb{D} \rightarrow \mathbb{R}
    \end{equation*}
\end{definition}
...
Popular examples of algorithms are: k-means \cite{}, isolation forest \cite{}

\begin{example}[K-Means \cite{}]
...
\begin{example}[Silhouette Index \cite]\label{ex:sil}
...
Given a dataset $\altmathcal{D} = \{d_1, \dots, d_n\} \in \mathbb{D}$, if the task $T$ is supervised, we refer with $\altmathcal{D}_{train}$ and $\altmathcal{D}_{valid}$ to two different data splits; otherwise, if $T$ is unsupervised, they correspond to the same original dataset $\altmathcal{D}$.
Then, given a loss function $\altmathcal{L} = \altmathcal{H} \times \mathbb{D} \rightarrow \mathbb{R}$, we define the following optimization problems.

\begin{definition}[Algorithm Selection (AS)]
    Given a set of learning algorithms $\altmathcal{A} = \{A_1, \dots, A_m\}$, the goal of algorithm selection is to determine the algorithm $A^{\star} \in \altmathcal{A}$ with optimal loss function
    \begin{equation*}
        A^{\star} \in \argmin_{A \in \altmathcal{A}} \altmathcal{L}(H, \altmathcal{D}_{valid})
    \end{equation*}
    where $H$ is the model trained by $A \in \altmathcal{A}$ with its default hyperparameter setting on $\altmathcal{D}_{train}$.
\end{definition}
...
\begin{definition}[Hyperparameter Optimization (HPO)]
    ...
    \begin{equation*}
        \pmb{\lambda}^{\star} \in \argmin_{\pmb{\lambda} \in \pmb{\Lambda}} \altmathcal{L}(H_{\pmb{\lambda}}, \altmathcal{D}_{valid})
    \end{equation*}
\end{definition}

There has been considerable past work separately addressing algorithm selection \cite{} and hyperparameter optimization \cite{}.
Yet, the combined space of learning algorithms and their hyperparameters is very challenging to search: the corresponding loss function becomes noisy when the space is high-dimensional, involves both categorical and continuous choices, and contains hierarchical dependencies (e.g., the hyperparameters of a learning algorithm are only meaningful if that algorithm is chosen).

\begin{definition}[Combined Algorithm Selection and Optimization (CASH)]
    Given a set of learning algorithms $\altmathcal{A} = \{A_1, \dots, A_m\}$, with assocciated hyperparameter space  $\pmb{\Lambda}_1, \dots, \pmb{\Lambda}_m$, the goal is to find the algorithm and corresponding configuration $A^{\star}_{\pmb{\lambda}^{\star}} \in \pmb{\Lambda}$ with optimal loss function.
    \begin{equation*}
        A^{\star}_{\pmb{\lambda}^{\star}} \in \argmin_{A_i \in \altmathcal{A}, \pmb{\lambda} \in \pmb{\Lambda}_i} \altmathcal{L}(H_{\pmb{\lambda}}, \altmathcal{D}_{valid})
    \end{equation*}
\end{definition}

% Dire -- come nel paper di weka -- che CASH può essere affrontato in molti modi (in molti ci hanno provato: algoritmi genetici, multi-fidelity) ma da quando c'è bayesian optimization si hanno risultati
% Dire che la loss è considerata objective
% fare la differenzazione tra single e multi objective
