...
% The sample consists of some data items X, instances such as sensor measurements, each of them characterized by specific features F, properties like intensity, duration, or location.
% The aim is to understand how changes in these features within X translate into changes in a desired output Y, like crop yield
% Intuitively, we are provided with some data points $X$ -- such as sensor measurements -- characterized by specific features -- like intensity, duration, or location -- and we are interested in understanding how changes in these features translate into changes in the outcome $Y$---like crop yield.
% Intuitively, we are provided with some data points $X$, such as sensor measurements, which are characterized by specific features like intensity, duration, or location.
% We are interested in understanding how changes in these features translate into changes in the outcome $Y$, such as the crop yield.
% While in supervised learning we collect the desired outcomes along with $X$, in unsupervised learning we are not aware of such information.
% Intuitively, we are provided with some data points $X$, such as sensor measurements, which are characterized by specific features like intensity, duration, or location.
% While in supervised learning we are provided with the desired outcomes $Y$, such as the crop yield; in unsupervised tasks, we are not aware .
% As mentioned, in supervised learning, data items $X$ come with the corresponding correct outcomes $Y$; while, in unsupervised learning, no ground truth is provided.
...
\begin{definition}[Dataset]\label{def:dataset}
    ...
    % , characterized by a set of features $\altmathcal{F}$.
    % According to the task $T$, a ground truth $Y$ -- within its corresponding space $\altmathcal{Y}$ -- is available in the dataset (supervised learning) or not (unsupervised learning).
    % According to the task $T$, the dataset includes the ground truth $Y$, with corresponding space $\altmathcal{Y}$ (supervised learning); or it is left as it is (unsupervised learning).
...
% Intuitively, data points $X$ -- such as sensor measurements -- are characterized by specific features -- like intensity, duration, or location -- and we are interested in understanding how changes in these features affect some outcome of interest $Y$---like crop yield.
% While in supervised learning we can drive algorithms by providing them with the desired outcomes, unsupervised tasks are inherently more exploratory and algorithms have to autonomously discover patterns and relationships within the data.
...
% The literature further differentiates the tasks contingently to the domain of $Y$.
% In supervised learning, we consider the outcome as a \textit{class} when it can assume finite values, and hence talk about \textit{classification}; otherwise, when the outcome is continuous, we refer to \textit{regression}.
% Analogously in unsupervised learning, we refer to \textit{clustering} when searching for different finite clusters---or groups, partitions.
% Otherwise, the nomenclature usually comes after the task semantic; for example, anomaly detection assigns an outlier score to each instance that deviates from the overall distribution.
...
% Yet, this does not affect our formalization.
...
% Finally, we need a loss function $\altmathcal{L}: \altmathcal{H} \times \mathbb{D} \rightarrow \mathbb{R}$ quantifying how well a given model performs on a given dataset.
% Although we provide a formalization allowing supervised and unsupervised cases to be treated likewise, it is worth noticing that -- under the hood -- algorithm functioning and performance assessment work differently according to the task at hand.
...
% In supervised learning, the original dataset $\altmathcal{D}$ is split into two disjoint partitions $\altmathcal{D}_\mathit{train}$ and $\altmathcal{D}_\mathit{valid}$.
% The model is trained only based on $\altmathcal{D}_\mathit{train}$ and validated with $\altmathcal{L}$ on $\altmathcal{D}_\mathit{valid}$.
% This prevents the algorithm to overfit 

% In supervised learning, the dataset is split into different partitions: $\altmathcal{D}_{train}$, $\altmathcal{D}_{valid}$, $\altmathcal{D}_{test}$,
% algorithms are trained with the aid of the ground truth $Y$
