...
The possibility to accurately and precisely forecast  agricultural systems 
can provide significant improvement in a variety of agricultural issues such as irrigation demand, drought, climate change, nutrient depletion and soil degradation \cite{vitali2021crop}. In \cite{silva-giller-2020} the authors presented a series of relevant questions regarding the ability of crop models to address these issues. They pointed out
that during the Second International Crop Modelling Symposium (iCropM2020), a keyword search in the book of abstracts showed that the term ‘simulation’ outweighed by far the terms ‘experiments’, ‘trials’ and ‘data acquisition’, and few contributions dealt with model improvement. The authors stressed the importance of coupling experimental data and models for an effective application of crop models at the operational level. The uncertainty associated with crop modelling computation can be reduced by coupling crop models with real time acquisition of relevant soil and crop variables. Today, with digitalization and remote acquisition of real time data it is possible to effectively incorporate experimental data and modelling in a real time framework. 

Moreover, many crops are still under-represented in crop models, including tropical perennials and fruit trees. Among fruit trees, the kiwifruit (\emph{Actinidia deliciosa}) has been only partially studied and in particular for a correct quantification of its water demands. 
The kiwifruit, often shortened to kiwi (used hereafter), is an edible fruit, originated in China, and widely diffused in many countries. Italy is the third largest producer of kiwi worldwide, with an yearly production of 524,490 MTons \cite{fao-2020}. Since it originated in areas with large availability of water (central and eastern China), this plant has high water demand and it is necessary to correctly asses its soil water budget in several pedo-climatological conditions, for an effective use of water resources. 
For instance in the Emilia-Romagna region (Italy), the kiwi irrigation season in generally starts in May and ends in October \cite{villani-etal-2011}, therefore with high water demands in a period where droughts and water scarcity are becoming more frequent due to climate change. While these effects of climate change also applies to other European regions \cite{olesen-etal-2011}, Europe's temperature increases and precipitation decreases are more pronounced in southern Europe \cite{ec-2013}, having adverse impacts on agriculture \cite{bindi-olensen-2011}.
In a study on irrigation demand projections in kiwi, \cite{villani-etal-2011}
reported that for the 2021-2050 period, with respect to the 1961-1990 period, a general
...
Few research papers have been published on measuring and modelling kiwi water demand. Indeed, in a review on fruit tree crop models, \cite{grisafi-etal-2021} showed that only one research was published on modelling kiwi and with emphasis on carbon budget, but not on the soil water budget. Little has also been done in terms of modelling the plant's water use and transpiration. \cite{rallo-etal-2020} presented a review about FAO56 crop coefficients of fruit trees and vines, performed over the past twenty years, to update information and extend tabulated single and basal standard crop coefficients. Also in this review it appears that kiwi is highly under-represented with only one listed research by \cite{silva-etal-2008}, where kiwi's transpiration was measured with a sap flow method. 
...
A correct irrigation forecasting must rely on a robust water budget model able to compute the soil water content (SWC) and soil water potential (SWP) in the root zone, and provide an estimated soil water requirement to keep the cultivated plant in optimal conditions. 
A water budget model implies the measurement and/or the computation of a variety of processes including surface runoff, deep percolation into the groundwater, capillary rise from the groundwater, subsurface lateral flow, soil evaporation and plant transpiration \cite{marshall-etal-1996}. Measurement of all these variables is usually not feasible for operational conditions in real-world farms, therefore simulation models play a fundamental role along with measurements of weather data and a few, key soil variables. Weather data can be easily provided with low cost portable weather stations, or by downloading data from weather data networks. Weather forecast is provided by local public agencies or by private companies. It is not difficult today to obtain a relatively low cost service for weather forecasting. Soil properties such as SWC and SWP are easily measured by installing low cost sensors in the soil profile 
\cite{bittelli-2010,nagahage-etal-2019}.


%DATA ASSIMILATION
The challenging aspect for a correct simulation of the water budget is the parameterization and corroboration of the simulation model. Parameterization can be obtained from experimental determination of soil and plant properties. For instance, the soil hydraulic properties (the soil water retention and hydraulic conductivity curves) can be measured in the laboratory. However, these measurements are expensive and time consuming. It is rarely a feasible solution for farmers, to measure the soil hydraulic properties. 
...
\begin{itemize}
	\item \olab{}, an innovative \textit{3D numerical model} specifically devised to compute the soil water balance of an orchard. The 3D model keeps into account: non uniform distribution of the root system, growth and development of orchard plants with a full computation of the soil water budget, short term weather forecast.
...
The following subsections describes in more details each of the components reported in \Cref{orchard-fig:overview}.
...
We propose an innovative numerical simulation model specifically devised for computing the soil water budget in an orchard.
The numerical solution is based on the integrated finite difference (also called cell-centered finite volume scheme) method.
\olab{} builds on \textit{Criteria-3D} \cite{bittelli-etal-2015} and introduces the following features in the computation of the soil water budget in an orchard: 
\begin{itemize}
...
This is used as the initial state of \olab{}, in this way the forecast begins from an actual soil condition (\Cref{orchard-fig:moisture-cont}).
...
Due to the size, energy requirement, and cost of sensors, it is not feasible to adopt sensor grids as fine-grained as the ones simulated by the \olab{}; in other words the number of simulated cells ($|SM|$) is sensibly greater than the number of in-situ sensors.
In \cite{DBLP:journals/cea/FranciaGG22}, we introduced several \textit{profiling functions} that approximate the soil moisture sampled by a coarse-grained 2D/3D sensor grid to a \textit{refined grid} with finer granularity (\Cref{orchard-fig:moisture-profile}).
...
Due to the huge number of parameters (\Cref{tab:parameter}), manual or brute-force tuning of \olab{} is unfeasible.
...
\Ensure{$X_{tun}^*$: best parameter configuration}
    ...
    \While{$iteration < budget$} \Comment{While some budget remains}
        ...
        \State \SMt $\gets \textup{\olab{}}(SM_{Ass}$, \Wt, \It, $X'_{tun} \cup X_{fix})$ \Comment{... run the simulator}
...
To the best of our knowledge, \olab{} is the only numerical model that leverages this innovative family of optimization methods.
...
\Cref{orchard-fig:forecasting_avg} reports the \textit{error} metric in \Cref{orchard-eq:error} for the three different forecasting horizons.
...
Overall, the average of the error is kept at: 0.27 for the 1-day horizon, 0.43 for the 3-days horizon, and 0.55 for the 7-days horizon. %; which translates into a forecast that varies -- respectively -- 30\%, 50\%, and 70\% of observed ground potential. 
%
...
\Cref{orchard-fig:stacked} and \Cref{orchard-fig:profiles}  shows a graphical representation of the measured soil moisture profile and compare it with the forecasting results. Colors emphasize water potential values according to five crop-specific soil moisture ranges (suggested by agronomist during the Agro.Big.Data.Science project \cite{ABDS}): 
