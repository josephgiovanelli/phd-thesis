...
\begin{example}[Silhouette Index \cite{sil}]\label{ex:sil}
    ...
    \end{equation*}
    where $C_i$ is the cluster of instances to which $x_i$ belongs i.e., $C_i = \{\pmb{x} \in \altmathcal{D} | H_{\pmb{\lambda}}(\pmb{x}) = H_{\pmb{\lambda}}(\pmb{x_i})\}$.
\end{example}
...
We provide the most common approaches for tackling single-objective optimization, in particular: from the baselines \textit{exhaustive}, \textit{grid}, and \textit{random search} to more sophisticated techniques from the \textit{model-free} family such as \textit{simulated annealing}, and the approaches based on \textit{evolutionary} and \textit{multi-fidelity}.
Finally, we deepen the \textit{model-based} framework of \textit{bayesian optimization} (BO).
Model-based approaches differentiate themselves from model-free ones in leveraging a surrogate model that drives the optimization toward the objective function and BO is the current leader of black-box optimization techniques.
...
As one might expect, we talk about bayesian optimization due to the well-known bayesian theorem in which it is possible to describe the probability of an event, based on prior knowledge of conditions that might be related to the event:
\begin{equation*}
    P(c | h) = \frac{P(h | c) \cdot P(c)}{P(h)}
\end{equation*}
Similarly, we can get the estimated cost $c$ of a hyperparameter configuration $\pmb{\lambda}$ given the history of previously evaluated configurations $h$.
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \While{$B$ has not been exhausted}
            ...
            \State Compute $c = \altmathcal{L}(A_{\pmb{\lambda}}(\altmathcal{D}_{train}), \altmathcal{D}_{valid})$
...
An example of prominent acquisition function is the probability of improvement $PI = P(c(\pmb{\lambda}) \ge c(\pmb{\lambda}^{\star}))$, where $\pmb{\lambda}^{\star}$ corresponds to the best hyperparameter configuration encountered so far during the optimization.

Yet, the real hype for bayesian framework relies on the improvements made in implementing extremely-performing surrogate models.
Gaussian processes are commonly considered the pioneers among the surrogates \cite{112, 80}, but they suffer from cubic complexity with the number of data points, limiting parallelization capability.
Tree-Structured Parzen Estimator (TPE) \cite{13}, in contrast, does not define a predictive distribution over the objective function but creates two density functions as generative models for all domain variables.
Given a percentile, observations are partitioned into two sets (good observations and bad observations), where simple \textit{parzen windows} model the two sets.
The ratio between the two density functions reflects the improvement in the acquisition function and is used to recommend new configurations for hyperparameters.
Tree-Structured Parzen Estimator (TPE) has demonstrated excellent performance in hyperparameter optimization tasks \cite{13, 12, 35, 37, 115}.
Random forests \cite{20} offer an alternative for modeling the objective function in Bayesian optimization.
The algorithm starts by growing $B$ regression trees, each built using $n$ randomly selected data points with replacement from training data of size $n$.
For each tree, a split node is chosen from $d$ algorithm parameters, and the minimum number of points considered for further split is set to $10$. The complexity of fitting and predicting variances for random forests is $O(n \log n)$ and $O(\log n)$, respectively, making them more scalable than Gaussian processes. Random forests are utilized by the Sequential Model-based Algorithm Configuration (SMAC) library \cite{51}.
