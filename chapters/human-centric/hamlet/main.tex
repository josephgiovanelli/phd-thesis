...
The remainder of the paper is structured as follows. In \Cref{sec:related}, we introduce the related work and necessary background. Then, we provide the problem formulation in \Cref{sec:problem} and its implementation in \Cref{sec:implementation}.
Finally, we provide some preliminary evaluation in \Cref{sec:test}, and we draw the conclusions and future research directions in \Cref{sec:conclusion}.
...
The module is also responsible for the generation and evaluation of the Problem Graph; it implements the Structured Argumentation functionalities as specified in \Cref{sec:problem} using \argtup{} \cite{arg2p-jlc}, an ASPIC\textsuperscript{+}-based Kotlin library.
...
In \Cref{sec:problem}, we defined the LogicalKB as the set of rules specified by the DS using her knowledge.
