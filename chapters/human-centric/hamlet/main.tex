...
Given a search space $\Lambda$ and a set of constraints $\altmathcal{C}$, finding the best pipeline instance $P^{\star}_{\lambda^{\star}}$ is defined as $P^{\star}_{\lambda^{\star}} = argmax_{\lambda \in \Lambda_P} metric(P_{\lambda})$, where $metric(P_{\lambda})$ is the function evaluating the goodness of $P_{\lambda}$ and the explored pipelines fulfill the constraints in $\altmathcal{C}$.
\end{definition}
