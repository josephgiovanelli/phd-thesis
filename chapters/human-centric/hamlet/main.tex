...
\begin{figure*}[t]
    ...
    \caption{Integrating HAMLET with the CRISP-DM process model.\todo{Adattare stile}\todo{Spostare più giù}}
...
We introduce HAMLET (Human-centric AutoMl via Logic and argumEnTation; \Cref{hamlet-fig:approach}), a framework that enhances AutoML with Structured Argumentation to:
...
\todo{Da invertire lambda}\todo{Il resto da rivedere dopo che si è toccati il background}
...
\begin{figure}[t]
    ...
    \end{lstlisting}
    \caption{A subset of rules from the LogicalKB.\todo{Qua perché non è più verde il commento?}}
    \label{rules-arg2p}
    \end{figure}
