...
% In Section \Cref{clustering-ssec:formalization} we formalize the approach and in \Cref{clustering-sec:implementation} we delve into the implementation.
...
%
% Given a hyperparameter configuration $\pmb{\aambda} \in \pmb{\Lambda}$, i.e., an (ordered) tuple in which each hyperparameter has been assigned with a value from its domain.
% $A_{\lambda}$
%
...
% including the possibility to return the dataset as-is through the ``Identity'' algorithm $I$.
%
% The step domain is defined as
...
% therefore the disjoint union of the domain of each algorithm.
% \footnote{If an algorithm has no hyperparameters ($\Lambda_{A} = \varnothing$), we set a placeholder $\Lambda_{A} = \{ 1 \}$.}. $\lambda_{S}$ denotes the algorithm instance selected for the step.
%
...
% We refer to an \textit{executable instance} as an ordered tuple of algorithm instances $P_{\lambda} = ({S_1}, \ldots, \lambda_{S_{|P|}}) \in \Lambda_P$.
