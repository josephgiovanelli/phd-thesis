...
\begin{figure*}[t]
    ...
    \caption{Data pre-processing and modeling challenges and corresponding problems in the literature.}
...
Once the data is transformed into the proper form, the modeling stage focuses on the combined algorithm selection and hyperparameter (CASH) optimization problem.
...
Some AutoML frameworks \cite{auto_sklearn, mohr2018ml}, mix-in pre-processing during modeling optimization, but typically include very few transformations or do not consider all the necessary steps (e.g., imputation, rebalancing), thus in a way overlooking it.
...
\begin{itemize}
    \item We empirically evaluate the impact of optimizing the exhaustive set of potential prototypes and find out that	there is no universal solution---i.e., a prototype that works best for each dataset and algorithm considered.
	...
	We exemplify our meta-learning study by generating simple but not obvious and effective rules for two kinds of pre-processing steps, namely, Feature Engineering and Rebalancing.
    ...
    \begin{itemize}
        ...
        \item on average, in 73\% of the cases, splitting evenly the time budget between pre-processing and modeling outperforms the results of solely optimizing the latter.
    \end{itemize}
	...
	The experiments were run on several machines and confirmed all the insights in this chapter.
\end{itemize}

The remaining of this chapter is organized as follows:
\Cref{effective-sec:related-work} discusses the related works,
...
At the beginning, the AutoML community followed the AI trend of contributing under a more algorithmic perspective, focusing on modeling---i.e., the resolution of the CASH problem.
...
On the other hand, some consider optimizing DPSO along with CASH, adopting a joint policy---data pre-processing in synergy with modeling (\Cref{effective-ssec:dpso-cash}).
...
The authors, inspired by AutoWeka, address the problem with the SMAC \cite{lindauer-jmlr22a}.
...
However, to keep the reader from losing, we will illustrate our use case and corresponding findings along with the explanation of the methodology (highlighted with the \textbf{Use Case} environment).
...
To bridge this gap, along with this work, we contribute to publishing a companion reproducibility paper \cite{giovanelli2023reproducible} for the experiments and results reported in the following.
...
That is, we want to examine whether dedicating a part of the optimization budget to the pre-processing impacts more (positively) the results of the analysis, than using the whole budget for solely the hyperparameter optimization of the ML algorithm\footnote{To enable the application of the ML algorithms on all the datasets, whenever required, we apply the necessary transformation (e.g, imputation or encoding).}---i.e., modeling.
