...
% \subsection{Combined Algorithm Selection and Hyperparameter optimization (CASH)}
...
% The dataset $D$ is divided into $D_{train}$ and $D_{test}$, to build and to evaluate the overall performance, calculated through the loss function $\mathcal{L}$. The problem is set up as an optimization problem and, as such, the configuration space is assumed to be known in advance (the set of algorithms $\{A^{1}, \dots , A^{k}\}$ and the related hyperparameter spaces $\{\Lambda^{1}, \dots, \Lambda^{k}\}$).
...
On the one hand, some works automate data pre-processing in isolation from modelling---i.e., addressing solely DPSO (\Cref{effective-ssec:dpso}).
% consider data pre-processing in isolation; that is, regardless of the task and analysis at hand.
% For instance,
% apply pre-processing for the sake of either guaranteeing data quality, enabling data exchange, or even data integration \cite{Llunatic13VLDBEnd,BigDansing15SIGMOD,Katara15SIGMOD,Foofah17SIGMOD}.
% Specifically, we are interested in those works that see pre-processing as an integral part of the learning process, and hence apply it for the sake of improving the final result of the ultimate analysis.
% In \Cref{effective-ssec:dpso}, we overview them.
On the other hand, some consider data pre-processing in synergy with modelling---i.e., combining DPSO and CASH (\Cref{effective-ssec:dpso-cash}).
% Finally, in \Cref{effective-ssec:dpso-cash}, we discuss the works that
% aim at fully automating the learning process, combining DPSO with CASH, where the border line between the two becomes blurry.
% Nevertheless, we tentatively group the works based on the type of problem they aim to solve.

% \subsection{CASH}
% \label{effective-ssec:cash}
% The task in solving the CASH problem is to automatically find an optimized instantiation for the hyperparameters of a set of ML algorithms.
...
% % The authors formalized the problem of algorithm selection and their associated hyperparameter optimization, and solved it in a combined search space.
...
% The one that achieves the higher accuracy is chosen.
% Rather than Bayesian Optimization, to find suitable hyperparameters, the authors utilize a basic evolutionary algorithm.
...
% It collects a large number of datasets and applies different ML algorithms (with different hyperparameters configurations).
...
% A data pipeline $P$ is a sequence of pre-processing steps that shape a dataset $\altmathcal{D}$ and a subsequent final ML step in which we address the task---in our case, either classification or regression.
...
% Qua ci va la formalizzazione dello spazio abnorme
...
% si cerca il prototipo (è più semplice)
...
% Data pre-processing is a challenging task due to the fact that there are no clear rules and guidelines in terms of which permutation of steps would work best for the prototype and which transformations -- with corresponding hyperparameters -- would have the highest impact on the result.
...
\subsection{Framework-related Rules Extraction}
...
\subsection{Heuristic Rules Discovery}
...
\begin{figure}
	...
	\ffigbox{
		\includegraphics[clip, trim=2.5cm 23cm 10.5cm 2cm,width=0.5\textwidth]{chapters/data-centric/supervised/img/graph.pdf}
		  ...
		  \caption{Precedence graph generated from \Cref{effective-tbl:rules-union}. $E$ - Encoding; $N$ - Normalization; $D$ - Discretization; $I$ - Imputation; $R$ - Rebalancing; $F$ - Feature Engineering. }
	}
	\capbtabbox{
	\begin{tabular}{@{}ll}
				\toprule
...
\subsection{Instantiation Rules (Meta-)Learning}
...
\begin{enumerate}
    ...
    \item Assess and compare the impact of dedicating a portion of the optimization time to the effective pipelines constructed using our method, with the impact of using the whole optimization time for the hyperparameters of the ML algorithm (\Cref{effective-sec:eval-dpso-vs-cash}).
\end{enumerate}
...
\begin{itemize}
    ...
    \item [--] Dedicating a portion of the time to the pre-processing optimization, instead of dedicating it entirely to hyperparameter optimization may boost %(have a higher impact on?!)
