...
On the other hand, in \Cref{effective-ssec:dpso}, we discuss the works that see pre-processing as an integral part of the learning process, and hence apply it for the sake of improving the final result of the ultimate analysis.

Finally, in \Cref{effective-ssec:dpso-cash}, we overview the works that aim at fully automating the learning process, combining DPSO with CASH, where the border line between the two becomes blurry.
...
\Cref{fig:methodology} sketches the proposed methodology.
...
% of the transformations within the prototype.
% Indeed, the process concludes with the two flow converging to the actual optimization.
% from the two flows is fed to the final step where the instantiation and optimization happens.
The result is an executable pipeline.

\paragraph{Effective Prototypes Selection} The proposed method starts with the selection of the ML library (\Cref{effective-ssec:select-framework}).
...
The output is a set of \textit{empirically learned rules} (\Cref{effective-ssec:rules-learned}) that determines yet other precedences of transformations, namely promising orderings that would potentially positively impact the final result of the analysis.
...
% When combining two different pre-processing steps, it is important to check if, (i) the input and output types of transformations are compatible, (ii) the combination makes sense, and (iii) the combination provides good results for the analysis.
% As a result, when chaining a pair of transformations, the following precedence relationships arise:

% \begin{enumerate}
%     \item Compatible/Incompatible pairs. Depending on whether the representation output of the first transformation is accepted as the representation input of the second one (compatible), or not (incompatible) (\Cref{effective-ssec:rules-framework}).
%     \item Meaningful/Meaningless pairs. Depending on whether the precedence between them makes sense based on generic knowledge (i.e., based on the literature) over the behaviour of transformations (meaningful), or not (meaningless) (\Cref{effective-ssec:rules-heuristics}).
%     \item Promising/Unpromising pairs. Depending on whether the precedence between them is expected to provide positive impact on the final result of the analysis (promising), or not (unpromising).
% \end{enumerate}

% Attending to the relationships between its transformations, a prototype can be described as either \textit{compatible}, \textit{well-formed}, or \textit{effective}.
...
% In fact, the ultimate goal of our method is to find \textit{effective pipelines}.
...
For our experiments, we selected the pre-processing steps, transformations and hyperparameters from those available in the scikit-learn\footnote{\url{https://scikit-learn.org}} library, and they are listed in \Cref{tbl:transformations}.
...
\begin{example}
We studied the transformations implemented in Scikit-learn, the discovered rules are shown through the adjacency matrix in \Cref{tbl:rules}a.
...
For the rest of the pairs of transformations, there are no constraints imposed by the framework, thus any order of such transformations is permitted, reflected by a \texttt{0} in \Cref{tbl:rules}a.
