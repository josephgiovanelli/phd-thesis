...
\paragraph{Sequential model-based algorithm configuration (SMAC)} \cite{smac} introduce SMAC, leveraging Random forest \cite{20} as a regressor to model the posterior distribution of the surrogate i.e., the well-known ML model is used to estimate the cost as in a usual regression task.
...
However, determining the weights can be challenging, especially when there is uncertainty or conflicting opinions about the relative importance of objectives. Dynamic weighted aggregation \cite{} addresses this issue by iteratively adjusting the weights to explore different trade-offs.
