...
\begin{definition}[Data Pipeline Selection and Optimization (DPSO)]
    ...
    \begin{equation*}
        P^{\star}_{\pmb{\lambda}^{\star}} \in \argmin_{P_{\pmb{\lambda}} \in \pmb{\Lambda}} \altmathcal{L}(\langle P_{\pmb{\lambda}}, A \rangle (\altmathcal{D}_{train}), \altmathcal{D}_{valid})
    \end{equation*}
...
% In the first phase, the optimizer (e.g., BO) is used to build the data pipeline.
% During the second phase, the optimizer is used to configure the algorithm.
