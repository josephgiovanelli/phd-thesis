...
\begin{example}[Iris dataset \cite{Unwin2021TheID}]\label{ex:dataset}
...
\begin{example}[K-Means \cite{arthur2006k}]
...
\begin{example}[Silhouette Index \cite{zhu2010clustering}]\label{ex:sil}
...
\begin{definition}[Algorithm Selection (AS) \cite{kerschke-ec19a}]
...
\begin{definition}[Hyperparameter Optimization (HPO) \cite{levesque-ieee17a}]
...
\begin{definition}[Combined Algorithm Selection and Optimization (CASH) \cite{Thornton13AutoWeka}]
...
\noindent This begins by building a surrogate model $S$ that captures the dependence of the loss function $L$ on hyperparameter settings $\pmb{\lambda}$ (line 1).
It then iterates through the following steps: using $S$ to determine a promising candidate configuration of hyperparameters $\pmb{\lambda}$ to evaluate next (line 3); evaluating the loss $c$ of $\pmb{\lambda}$ (line 4); and updating the surrogate model $S$ with the new data point $(\pmb{\lambda}, c)$ obtained (lines 5–6).
To select its next hyperparameter configuration $\pmb{\lambda}$ using the surrogate model $S$, SMBO employs a so-called acquisition function $a_{S} : \pmb{\Lambda} \rightarrow \mathbb{R}$, which uses the predictive distribution of $S$ at arbitrary hyperparameter configurations $\pmb{\lambda} \in \pmb{\Lambda}$ to quantify (in closed form) how useful knowledge about $\pmb{\lambda}$ would be.
...
\paragraph{Tree-structured Parzen Estimator (TPE)} In contrast, TPE does not define a predictive distribution over the objective function but creates two density functions for all the hyperparameters in the problem.
...
\paragraph{Sequential Model-based Algorithm Configuration (SMAC)} \cite{lindauer-jmlr22a} introduce SMAC, leveraging  forest \cite{random_forest} as a regressor to model the posterior distribution of the surrogate i.e., the well-known ML model is used to estimate the cost as in a usual regression task.
...
Authors privde also other state-of-the-art BO implementations; for instance, in \cite{falkner2018bohb}, they propose bayesian optimization and hyper band (BOHB), which enhances the global exploration capabilities of BO with the efficient resource utilization of multi-fidelity functions.
...
% These methods often rely on a priori information from the decision-maker, such as preferences or target values for each objective.
...
As in the CRISP-DM model, the learning process not only involves algorithm training -- i.e., modeling -- but also data preparation, better known as data pre-processing.
...
After having fitted the training parameters, this corresponds to the sequence of models in \Cref{background-def:data_pipeline}.
...
\begin{definition}[Machine Learning Pipeline]
...
The budget is split between pre-pocessing and modeling, allocated respectively for the data pipeline configuration search and the algorithm hyperparameter tuning.
